...
/**
 * Test for Magento\GiftCardAccount\Observer\PaymentDataImport.
 */
class PaymentDataImportTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testExecuteAvailableGiftCards()
    {
        ...
        $giftCardAccountMock = $this->getMockBuilder(\Magento\GiftCardAccount\Model\Giftcardaccount::class)
            ->disableOriginalConstructor()
            ->setMethods([
                'loadByCode',
                'isValid',
            ...
            ->getMock();
        $giftCardAccountMock->expects($this->exactly(2))
            ->method('loadByCode')
            ->with($giftCardCode)
            ->willReturnSelf();
        ...
        $this->getCardAccountFactory->expects($this->exactly(2))
            ->method('create')
            ->willReturn($giftCardAccountMock);
