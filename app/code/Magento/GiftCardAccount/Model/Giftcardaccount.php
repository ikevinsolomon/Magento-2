    ...
    implements GiftCardAccountInterface
{
    ...
    {
        return $this->getData(self::GUEST_EMAIL);
    }
     ...
     * {@inheritdoc}
     */
    public function setGuestEmail(array $email)
    {
     ...
     * @throws \Magento\Framework\Exception\LocalizedException
     */
    public function guestAddToCart($saveQuote = true, $quote = null, $email = null)
    {
        if (is_null($quote)) {
            $quote = $this->_checkoutSession->getQuote();
        }
        $website = $this->_storeManager->getStore($quote->getStoreId())->getWebsite();
        if ($this->isGuestValid(true, true, $website, true, $email)) {
            $cards = $this->_giftCardAccountData->getCards($quote);
            if (!$cards) {
                $cards = [];
            } else {
    ...
    public function isValid($expirationCheck = true, $statusCheck = true, $websiteCheck = false, $balanceCheck = true)
    {
		...
		$currentCustomerEmail = $this->_customerSession->getCustomer()->getEmail();
		//die();
		if ($this->_customerSession->isLoggedIn()) {
			if (!$this->getId() || $recipientEmail != $currentCustomerEmail) {
				$this->_throwException(
					__('Please correct the gift card account ID. Requested code: "%1"', $this->_requestedCode)
				);
			}

			}else if($guestCustomerEmail != ""){
			if (!$this->getId() || $recipientEmail != $guestCustomerEmail) {
				$this->_throwException(
					__('Please correct the gift card account ID. Requested code: "%1"', $this->_requestedCode)
				);
			}
     ...
     * @SuppressWarnings(PHPMD.NPathComplexity)
     */
    public function isGuestValid($expirationCheck = true, $statusCheck = true, $websiteCheck = false, $balanceCheck = true, $email = null)
    {
		$recipientEmail = $this->getRecipientEmail();
		$currentGuestCustomerEmail = $email;
		//die();
		if ($this->_customerSession->isLoggedIn()) {
			if (!$this->getId() || $recipientEmail != $currentGuestCustomerEmail) {
				$this->_throwException(
					__('Please correct the gift card account ID. Requested code: "%1"', $this->_requestedCode)
				);
			}
		}else if($currentGuestCustomerEmail != ""){
			if (!$this->getId() || $recipientEmail != $currentGuestCustomerEmail) {
				$this->_throwException(
					__('Please correct the gift card account ID. Requested code: "%1"', $this->_requestedCode)
				);
    ...
    public function redeem($customerId)
    {
        if ($this->isValid(true, true, true, true)) {
            ...
            $additionalInfo = __('Gift Card Redeemed: %1.', $this->getCode(), $customerId);
