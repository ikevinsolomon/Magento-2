...
namespace Magento\Seller\Ui\Component\Listing\Column;
...
class GroupActions extends Column
{
     ...
     * Url path
     */
    const URL_PATH_EDIT = 'seller/group/edit';
    const URL_PATH_DELETE = 'seller/group/delete';
    ...
    public function prepareDataSource(array $dataSource)
    {
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                if (isset($item['seller_group_id'])) {
                    $title = $this->escaper->escapeHtml($item['seller_group_code']);
                    $item[$this->getData('name')] = [
                        'edit' => [
                            'href' => $this->urlBuilder->getUrl(
                                static::URL_PATH_EDIT,
                                [
                                    'id' => $item['seller_group_id']
                                ]
                            ),
                            'label' => __('Edit'),
                        ],
                    ...
                    if (!$this->groupManagement->isReadonly($item['seller_group_id'])) {
