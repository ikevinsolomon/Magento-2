...
class Confirmation extends Column
{
    ...
    /**
     * Retrieve is confirmation required flag for seller considering requested website may not exist.
     *
     * @param array $seller
     * @return bool
     */
    private function getIsConfirmationRequired(array $seller): bool
    {
        try {
            return $this->accountConfirmation->isConfirmationRequired(
                $seller['website_id'][0] ?? null,
                $seller[$seller['id_field_name']],
                $seller['email']
            );
