 ...
 */
namespace Magento\Seller\Model\Account;
...
class Redirect
{
    ...
    /**
     * Update last seller id, if required
     *
     * @return void
     */
    protected function updateLastSellerId()
    {
        $lastSellerId = $this->session->getLastSellerId();
        if (isset($lastSellerId)
            && $this->session->isLoggedIn()
            && $lastSellerId != $this->session->getId()
        ) {
            $this->session->unsBeforeAuthUrl()
                ->setLastSellerId($this->session->getId());
        }
    }
        ...
        switch ($url) {
            case $baseUrl:
                if ($this->session->isLoggedIn()) {
                    $this->processLoggedSeller();
                } else {
                    $this->applyRedirect($this->sellerUrl->getLoginUrl());
                }
                break;

            case $this->sellerUrl->getLogoutUrl():
                $this->applyRedirect($this->sellerUrl->getDashboardUrl());
                break;
    ...
    /**
     * Prepare redirect URL for logged in seller
     *
     * Redirect seller to the last page visited after logging in.
     *
     * @return void
     */
    protected function processLoggedSeller()
    {
        // Set default redirect URL for logged in seller
        $this->applyRedirect($this->sellerUrl->getAccountUrl());

        if (!$this->scopeConfig->isSetFlag(
            SellerUrl::XML_PATH_SELLER_STARTUP_REDIRECT_TO_DASHBOARD,
            ScopeInterface::SCOPE_STORE
        )
        ) {
            $referrer = $this->request->getParam(SellerUrl::REFERER_QUERY_PARAM_NAME);
            if ($referrer) {
