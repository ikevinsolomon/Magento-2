...
{
    /**
     * Config paths to VAT related seller groups
     */
    const XML_PATH_SELLER_VIV_INTRA_UNION_GROUP = 'seller/create_account/viv_intra_union_group';

    const XML_PATH_SELLER_VIV_DOMESTIC_GROUP = 'seller/create_account/viv_domestic_group';

    const XML_PATH_SELLER_VIV_INVALID_GROUP = 'seller/create_account/viv_invalid_group';

    const XML_PATH_SELLER_VIV_ERROR_GROUP = 'seller/create_account/viv_error_group';
     ...
     */
    const XML_PATH_SELLER_GROUP_AUTO_ASSIGN = 'seller/create_account/auto_group_assign';
    ...
    /**
     * Retrieve seller group ID based on his VAT number
     *
     ...
     * @return null|int
     */
    public function getCustomerGroupIdBasedOnVatNumber($sellerCountryCode, $vatValidationResult, $store = null)
    {
        $groupId = null;

        $isAutoGroupAssign = $this->scopeConfig->isSetFlag(
            self::XML_PATH_SELLER_GROUP_AUTO_ASSIGN,
            ScopeInterface::SCOPE_STORE,
            $store
        );
        ...
        $vatClass = $this->getCustomerVatClass($sellerCountryCode, $vatValidationResult, $store);

        $vatClassToGroupXmlPathMap = [
            self::VAT_CLASS_DOMESTIC => self::XML_PATH_SELLER_VIV_DOMESTIC_GROUP,
            self::VAT_CLASS_INTRA_UNION => self::XML_PATH_SELLER_VIV_INTRA_UNION_GROUP,
            self::VAT_CLASS_INVALID => self::XML_PATH_SELLER_VIV_INVALID_GROUP,
            self::VAT_CLASS_ERROR => self::XML_PATH_SELLER_VIV_ERROR_GROUP,
        ];
     ...
     * @return null|string
     */
    public function getCustomerVatClass($sellerCountryCode, $vatValidationResult, $store = null)
    {
        $vatClass = null;

        $isVatNumberValid = $vatValidationResult->getIsValid();

        if (is_string($sellerCountryCode)
            && !empty($sellerCountryCode)
            && $sellerCountryCode === $this->getMerchantCountryCode($store)
            && $isVatNumberValid
        ) {
