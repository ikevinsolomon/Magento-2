 ...
 *
 *  Used to track sessions of the logged in sellers
 *
...
{
    const VISITOR_TYPE_SELLER = 'c';
    const VISITOR_TYPE_VISITOR = 'v';
    const DEFAULT_ONLINE_MINUTES_INTERVAL = 15;
    const XML_PATH_ONLINE_INTERVAL = 'seller/online_sellers/online_minutes_interval';
    ...
    /**
     * Bind seller data when seller login
     *
     * Used in event "seller_login"
     *
     * @param EventObserver $observer
     * @return Visitor
     */
    public function bindSellerLogin($observer)
    {
        /** @var SellerInterface $seller */
        $seller = $observer->getEvent()->getSeller();
        if (!$this->getSellerId()) {
            $this->setDoSellerLogin(true);
    ...
    /**
     * Bind seller data when seller logout
     *
     * Used in event "seller_logout"
     *
     * @param EventObserver $observer
     * @return Visitor
     * @SuppressWarnings(PHPMD.UnusedFormalParameter)
     */
    public function bindSellerLogout($observer)
    {
        if ($this->getSellerId()) {
            $this->setDoSellerLogout(true);
        }
        return $this;
    }
