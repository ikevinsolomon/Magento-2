 ...
 */
namespace Magento\Seller\Model\Metadata\Form;

use Magento\Seller\Model\FileProcessor;
...
/**
 * Processes files that are save for seller.
 *
...
class File extends AbstractData
{
     ...
     * @param \Magento\Seller\Model\FileProcessorFactory|null $fileProcessorFactory
    ...
    public function __construct(
        ...
        \Magento\Seller\Model\FileProcessorFactory $fileProcessorFactory = null,
        IoFile $ioFile = null
    ) {
    ...
    public function compactValue($value)
    {
        ...
        if (!empty($this->_value)
            && (!empty($value['delete'])
                || ($this->_entityTypeCode == 'seller' && empty($value)))
        ) {
     ...
     * @inheritdoc
     */
    public function outputValue($format = \Magento\Seller\Model\Metadata\ElementFactory::OUTPUT_FORMAT_TEXT)
    {
        $output = '';
        if ($this->_value) {
            switch ($format) {
                case \Magento\Seller\Model\Metadata\ElementFactory::OUTPUT_FORMAT_JSON:
