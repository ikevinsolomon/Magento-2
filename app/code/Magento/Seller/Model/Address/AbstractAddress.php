...
use Magento\Seller\Model\Data\Address as AddressData;
...
class AbstractAddress extends AbstractExtensibleModel implements AddressModelInterface
{
     ...
     * @var string
     */
    protected $_eventPrefix = 'seller_address';
     ...
     * @var string
     */
    protected $_eventObject = 'seller_address';
    ...
    {
        $name = '';
        if ($this->_eavConfig->getAttribute('seller_address', 'prefix')->getIsVisible() && $this->getPrefix()) {
            $name .= $this->getPrefix() . ' ';
        }
        $name .= $this->getFirstname();
        $middleName = $this->_eavConfig->getAttribute('seller_address', 'middlename');
        if ($middleName->getIsVisible() && $this->getMiddlename()) {
            $name .= ' ' . $this->getMiddlename();
        }
        $name .= ' ' . $this->getLastname();
        if ($this->_eavConfig->getAttribute('seller_address', 'suffix')->getIsVisible() && $this->getSuffix()) {
            $name .= ' ' . $this->getSuffix();
        }
        return $name;
    }
     ...
     * $addressMapper = \Magento\Seller\Model\Address\Mapper type
    ...
    public function format($type)
    {
        ...
        $this->_eventManager->dispatch('seller_address_format', ['type' => $formatType, 'address' => $this]);
    ...
    public function getDataModel($defaultBillingAddressId = null, $defaultShippingAddressId = null)
    {
        ...
        if ($this->getSellerId() || $this->getParentId()) {
            $sellerId = $this->getSellerId() ?: $this->getParentId();
            $addressDataObject->setSellerId($sellerId);
    ...
    {
        return ($this->_eavConfig->getAttribute('seller_address', 'company')->getIsRequired());
    }
    ...
    {
        return ($this->_eavConfig->getAttribute('seller_address', 'telephone')->getIsRequired());
    }
    ...
    {
        return ($this->_eavConfig->getAttribute('seller_address', 'fax')->getIsRequired());
    }
