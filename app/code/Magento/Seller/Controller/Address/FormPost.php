...
/**
 * Seller Address Form Post Controller
 *
 * @SuppressWarnings(PHPMD.CouplingBetweenObjects)
 */
class FormPost extends \Magento\Seller\Controller\Address implements HttpPostActionInterface
{
    /**
     * @var RegionFactory
     */
    ...
    protected function _extractAddress()
    {
        ...
        /** @var \Magento\Seller\Model\Metadata\Form $addressForm */
        ...
        $addressDataObject->setSellerId($this->_getSession()->getSellerId())
            ->setIsDefaultBilling(
    ...
    protected function getExistingAddressData()
    {
        ...
        if ($addressId = $this->getRequest()->getParam('id')) {
            ...
            if ($existingAddress->getSellerId() !== $this->_getSession()->getSellerId()) {
                throw new NotFoundException(__('Address not found.'));
            }
            $existingAddressData = $this->getSellerAddressMapper()->toFlatArray($existingAddress);
        }
    ...
    protected function updateRegionData(&$attributeValues)
    {
        ...
        $this->dataObjectHelper->populateWithArray(
            $region,
            $regionData,
            \Magento\Seller\Api\Data\RegionInterface::class
        );
    ...
    /**
     * Get Seller Address Mapper instance
     *
     * @return Mapper
     *
     * @deprecated 100.1.3
     */
    private function getSellerAddressMapper()
    {
        if ($this->sellerAddressMapper === null) {
            $this->sellerAddressMapper = ObjectManager::getInstance()->get(
                \Magento\Seller\Model\Address\Mapper::class
            );
        }
        return $this->sellerAddressMapper;
    }

    /**
     * Removes file attribute from seller address and file from filesystem
     *
    ...
    private function deleteAddressFileAttribute($address)
    {
        ...
        if ($attributeValue!== null) {
            if ($attributeValue->getValue() !== '') {
                ...
                $path = $mediaDirectory->getAbsolutePath('seller_address' . $fileName);
