...
class CategoryUrlPathAutogeneratorObserver implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($category->getUrlKey() !== false && !$useDefaultAttribute) {
            $resultUrlKey = $category->getUrlKey();
    ...
    private function updateUrlKey(Category $category, ?string $urlKey): void
    {
        ...
        $eventData = [];
        $eventData['category_id'] = $category->getId();
        $eventData['event_name'] = self::CATALOG_CATEGORY_RESOURCE;
        $eventData['updated_url_key'] = $urlKey;
    ...
    {
        $invalidValues = !is_null($this->frontNameResolver) ? array_unique(array_merge($this->invalidValues, [$this->frontNameResolver->getFrontName()])) : [];
